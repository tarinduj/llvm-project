  ...
  if (any_isa<Function *>(Any(&IR))) { 
      ...
      if (!FnOptLevel.empty()) {
        ...
        if (OptimizationLevels.empty() || OptimizationLevels.count(OptLevel)) {
          ShouldRun = true; // apply the pass if the "Function Optimization Level" is
                       // in the "Pass Optimization Levels Set"
        } else {
          // dbgs() << "PASS SKIPPED!\n";
          ShouldRun = false; // skip the pass
        }
      } else {
        ShouldRun = true; // apply the pass if opt-level Attribute is empty
      }
  }
  return ShouldRun;
}
