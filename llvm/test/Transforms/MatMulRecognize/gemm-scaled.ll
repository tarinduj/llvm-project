...
;
; Test: Scaled GEMM pattern recognition.
;   for (i = 0; i < N; i++)
;     for (k = 0; k < N; k++)
;       for (j = 0; j < N; j++)
;         C[i][j] += alpha * A[i][k] * B[k][j];
;
; The pass should recognize this as a matmul with alpha scaling.
;
; CHECK: MMR: Recognized matmul
; CHECK: alpha

define void @gemm_scaled(ptr noalias %C, ptr noalias %A, ptr noalias %B,
                         float %alpha, i32 %N) {
entry:
...
for.j:
  ...
  ; C[i][j] += alpha * A[i][k] * B[k][j]
  %alpha.a = fmul float %alpha, %a.val
  %mul = fmul float %alpha.a, %b.val
